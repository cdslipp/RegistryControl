...
export async function fetchInitialShowMode() {
  const record = await pb.collection('show_mode').getOne(recordId);
  showMode.set(record.showModeStatus);
}
